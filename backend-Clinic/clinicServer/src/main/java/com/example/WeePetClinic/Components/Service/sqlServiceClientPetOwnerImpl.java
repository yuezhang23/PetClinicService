...
@Component
@Service
public class sqlServiceClientPetOwnerImpl implements ServiceClientPetOwner {
  ...
  public UserClientPetOwnerImpl getPetOwner(String id) {
    return accessPetOwner.findPetOwnerById(id);
  }
  ...
  @Override
  public List<ProjPetProfile> displayPetsInfo(UserClientPetOwnerOri owner) {
    try {
  ...
  public UserEmpVetImpl findVetByAppointment(int apt_id) {
    return accessPetOwner.findVetByAppointment(apt_id);
  }

  @Override
  public List<AppointmentOri> displayAppointmentsDetailsForPet(PetOri pet, UserClientPetOwnerOri petOwner) {
  ...
  @Override
  public List<InvoiceOri> displayInvoicesForAppointment(int appointId, UserClientPetOwnerOri petOwner) {
  ...
  @Override
  public List<InvoiceOri> displayAllInvoicesForUser(UserClientPetOwnerOri petOwner) {
    List<InvoiceImplSql> in1 =  accessPetOwner.findInvoiceByOwner(petOwner.getUserID());
    return new ArrayList<>(in1);
  }

  @Override
  public List<AppointmentOri> displayAppointmentsForUser(UserClientPetOwnerOri petOwner) {
    List<AppointmentOri> lst = new ArrayList<>();
    List<PetImpl> pets = petOwner.getPets();
  ...
  @Override
  public List<ProjAppointmentDetail> displayAppointmentsDetailsForUserInClinic(int clinicID, UserClientPetOwnerOri owner) {
  ...
  @Override
  public boolean accountCancellation(boolean delete, UserClientPetOwnerOri user) {
    try {
      if (delete) {
  ...
  @Override
  public boolean updateAccount(UserClientPetOwnerOri newAccount) {
    return false;
  }
