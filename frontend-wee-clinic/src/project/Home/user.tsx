...
// import {setPets} from '../../utils/redux/crudReducer'
...
// function UsrHome() {
//   const [carePets, setCarePets] = useState<any[]>([]);
...
//   // const searchPetsUserCared = async () =>{
//   //   if (!currentUser) {
//   //     alert('You are not authorized, please Sign In/Up!')
//   //     return
//   //   } 
//   //   const PetsFromUser = await common.findPetsUserCared(currentUser._id)
//   //   if (PetsFromUser.length === 0) {
//   //     alert('you have never liked/followed/commented on any Petery')
//   //   } else {
//   //     setCarePets(PetsFromUser)
//   //   }
//   // }
...
//   // const unLikes = async (Pet : any) =>{
//   //   // keep click button bug
//   //   if (!doneLike) {
//   //     const newPet = {...Pet, likeCount : Pet.likeCount - 1, likers : Pet.likers.filter((i :any) => i._id != currentUser._id)};
//   //     const newPets = currentPets.map((i)=> i._id === Pet._id? newPet : i);
//   //     admin.updatePet(Pet._id, newPet).then((status) => dispatch(setPets(newPets)));
//   //     SetLike(!doneLike);
//   //   }
//   // };
  
//   // const unFollowers = async (Pet : any) =>{
//   //   if (!doneFollow) {
//   //     const newPet = {...Pet, followers : Pet.followers.filter((i :any) => i._id != currentUser._id), followCount : Pet.followCount - 1};
//   //     const newPets = currentPets.map((i)=> i._id === Pet._id? newPet : i);
//   //     admin.updatePet(Pet._id, newPet).then((status) => dispatch(setPets(newPets)));
...
//         Welcome  <strong className='text-danger'> @ {  currentUser && currentUser.firstName }</strong>
//         <Link to = {`/Home/User`}
//         className='btn btn-outline-danger ms-3 rounded-4'> Home </Link>
//       </div>
//       <div className='px-5 fs-4 mb-3' > Peteries You ever <strong className='text-danger'>Liked/Followed/Commented</strong></div>
//       <ul className="list-group d-flex flex-grow-1 ps-5"> 
//           {carePets && carePets.map((br: any) => 
//            ( 
//             <li 
...
//                       <strong className='fs-5'> <FaEarlybirds/> Petery :  </strong>  <br></br>
...
//             </li>))
//             }  
//             <span className={carePets.length == 0 ? "text-danger m-5" : "d-none"}>
//               ----------- No Result -----------</span>
//       </ul>
